...
let repeatedLetter;
let matchedLetter;
sports = ["football", "soccer", "badminton", "crochet", "volleyball", "cricket", "raquetball", "rugby", "handball", "tennis", "boxing", "ice hockey", "karate"];
...
//fun random bonus thingy that has a rare occurence if the game decided to go on for further rounds
function luckyBonus(){
    if (scores[turn%3] >= 2500 && allowBonus === true) {
        let chance = Math.floor(Math.random() * 100 + 1);
        if (chance >= 85) {
            alert (PlayerNames[turn%3] + ", CONGRATULATIONS! Your score will be doubled!");
            scores[turn%3] = scores[turn%3] * 2;
            showMoney();
            allowBonus = false;
            turn++
            PlayerTurns();
...
function finalPuzzle(){
    ...
    tempBank = ["where the red fern grows", "general cornwallis", "central processing unit"]
    specificSpot = Math.floor(Math.random()*tempBank.length)
    word = tempBank[specificSpot];
    if (specificSpot == 0){
        document.getElementById("lastHint").innerHTML = "Hint: This is the title of a novel. Good luck. :)";
    } else if (specificSpot == 1) {
        document.getElementById("lastHint").innerHTML = "Hint: This general was kinda important in Amrican History";
    } else if (specificSpot == 2) {
        document.getElementById("lastHint").innerHTML = "Hint: there seems to be a shortage of these everywhere in the tech world";
    }
    wordArray = word.split(" ");
    console.log(wordArray);
    for (let e = 0; e < wordArray.length; e++) {
